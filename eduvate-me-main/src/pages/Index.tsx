...
import { MockScoreInput, MockPredictionInput } from '@/components/predictor/MockScoreInput';
import { CollegeResults } from '@/components/predictor/CollegeResults';
import { College } from '@/data/mockData';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { GraduationCap, TrendingUp, Users, Award, ArrowRight, FileText } from 'lucide-react';
...
export default function Index() {
  ...
  const [useMockScores, setUseMockScores] = useState(false);
  const navigate = useNavigate();
  const { toast } = useToast();

  // Map frontend exam types to backend exam types
  const getBackendExamType = (frontendExamType: string): string => {
    ...
    return examTypeMap[frontendExamType] || frontendExamType;
  };
  ...
  const handlePredict = async (input: PredictionInput) => {
    try {
      ...
      // Convert frontend exam type to backend exam type
      const backendExamType = getBackendExamType(selectedExam);

      // Call API
      const predictedResults = await collegesAPI.predict({
        rank,
        category: input.category,
        examType: backendExamType,
      });
  ...
  const handleMockPredict = async (input: MockPredictionInput) => {
    try {
      setLoading(true);
      ...
      // Call API for mock score prediction
      const predictedResults = await collegesAPI.predictFromMockScore({
        mockScore: input.mockScore,
      ...
      toast({
        title: "Prediction failed",
        description: error instanceof Error ? error.message : "Failed to predict colleges from mock score",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
            ...
            <div className="mt-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-display text-xl font-semibold">
                  {useMockScores ? 'Enter Mock Test Score' : 'Enter Your Score'}
                </h2>
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={() => setUseMockScores(!useMockScores)}
              ...
              {useMockScores ? (
                <MockScoreInput examType={selectedExam} onPredict={handleMockPredict} />
              ) : (
