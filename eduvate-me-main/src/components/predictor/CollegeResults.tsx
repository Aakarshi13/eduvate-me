...
import { MapPin, TrendingUp, IndianRupee, Users, ExternalLink, Plus, Info } from 'lucide-react';
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="truncate">
            Top recruiters: {Array.isArray(college.topRecruiters) ? college.topRecruiters.slice(0, 3).join(', ') : 'N/A'}
          </span>
        </div>
...
export function CollegeResults({ results, onAddToCompare }: CollegeResultsProps) {
  ...
  return (
    <div className="space-y-10">
      {/* Mock Score Metadata Display */}
      {results.metadata && Object.keys(results.metadata).length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-3">
            <Info className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
            <div>
              <h3 className="font-display font-semibold text-blue-900 mb-2">Based on Your Mock Test Score</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-xs text-muted-foreground">Your Score</p>
                  <p className="font-semibold">{results.metadata.mockScore} / {results.metadata.maxScore}</p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-xs text-muted-foreground">Percentile</p>
                  <p className="font-semibold">{results.metadata.percentile}%</p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-xs text-muted-foreground">Estimated Rank</p>
                  <p className="font-semibold">~{results.metadata.estimatedRank}</p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-xs text-muted-foreground">Prediction Basis</p>
                  <p className="font-semibold text-sm">Converted to Rank</p>
                </div>
              </div>
              <p className="text-sm text-blue-700 mt-2">
                Note: These predictions are based on converting your mock test score to an estimated rank. Actual results may vary.
              </p>
            </div>
          </div>
        </div>
          ...
          {(results[section.key] || []).length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {(results[section.key] || []).map((college, index) => (
                <CollegeCard 
                  key={college.id || `${section.key}-${index}`} 
                  college={college} 
